...
* Does the trait evolve under stabilizing selection?
* Given the trait undergoes stabilizing selection, what are the long-term optimum value and the rate of convergence towards it?
...
In reality, only the phenotypic value, at the tips, i.e. \code{z[1:N]} will be observable. One useful way to visualize the observed trait-values is to cluster the tips in the tree according to their root-tip distance, and to use box-whisker or violin plots to visualize the trait distribution in each group. This allows to visually assess the trend towards uni-modality and normality of the values - an important prerequisite for the POUMM. 
...
A somewhat fancier but not always useful way to visualize data is to plot the phylogeny with colored branches, scaling the intensity of the color by the trait value at the end of each branch:

```{r colored-tree-plots, fig.show = "hold", fig.height=4, fig.width=7}
# This is easily done using the nodeTimes utility function in combination with
# the cut-function from the base package.
...
# Biological interpretation of the POUMM fit
Let us come back to the question we stated at the beginning.

## Is the POUMM an appropriate model for the data?

Once we are convinced that the POUMM fits well to our data, we can go forward with the questions stated at the beginning. 

```{r fit-PMM}
cluster <- parallel::makeCluster(parallel::detectCores(logical = FALSE))
doParallel::registerDoParallel(cluster)

fitPMM <- poumm::PMM(z[1:N], tree, doMCMC = TRUE, spec=list(nSamplesMCMC = 4e5, parallelMCMC = TRUE))
...
```{r simulate-gez-BM}
# genotypic (heritable) values
gBM <- poumm::rVNodesGivenTreePOUMM(tree, g0, 0, theta, sigma)

# environmental contributions
eBM <- rnorm(length(gBM), 0, sigmae)

# phenotypic values
zBM <- gBM + eBM
```

```{r violin-plots-BM, fig.show = "hold", fig.height=4, fig.width=7}
# This is easily done using the nodeTimes utility function in combination with
# the cut-function from the base package.
data <- data.table(z = zBM[1:N], t = poumm::nodeTimes(tree, tipsOnly = TRUE))
data <- data[, group := cut(t, breaks = 5, include.lowest = TRUE)]
...
```{r fitPOUMM-PMM, eval=FALSE}
cluster <- parallel::makeCluster(parallel::detectCores(logical = FALSE))
doParallel::registerDoParallel(cluster)

fitPOUMM3 <- poumm::POUMM(zBM[1:N], tree, doMCMC = TRUE, spec=list(nSamplesMCMC = 4e5, parallelMCMC = TRUE))  

fitPMM <- poumm::PMM(zBM[1:N], tree, doMCMC = TRUE, spec=list(nSamplesMCMC = 4e5, parallelMCMC = TRUE))
...
```{r, fig.height=4, fig.show="hold", fig.width=7, warning=FALSE}
plot(fitPOUMM3, interactive = FALSE)
```

```{r, fig.height=4, fig.show="hold", fig.width=7, warning=FALSE}
plot(fitPOUMM3, startMCMC = 2e5, interactive = FALSE)
```

```{r, warning=FALSE}
summary(fitPOUMM3, startMCMC = 2e5)
```


```{r, fig.height=4, fig.show="hold", fig.width=7, warning=FALSE}
plot(fitPMM, interactive = FALSE)
```
