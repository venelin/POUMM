...
* parallelizing the POUMM likelihood calculation. The POUMM package parallelizes the likelihood calculation through the SPLiTTree library for parallel lineage traversal [@Mitov:2017ku]. This is a fine grain parallelization, which can benefit from modern single instruction multiple data (SIMD) processors as well as multiple physical CPU cores. Parallelization on multiple cores becomes beneficial on trees of exceeding several hundreds of tips. [@Mitov:2017ku]. For this parallelization to work, the POUMM package must be compiled from source-code using an OpenMP 4.0-enabled C++ compiler. Open MP 4.0 is supported by several modern C++ compilers including Gnu-g++ and Intel-icpc, but not the current version of the clang compiler. We have tested the package on Linux and MacOS X El Capitan using the Intel compiler v16.0.0. To that end, before installing the package from source, we modify the global ```Makevars``` file found in the directory ```.R``` under the user's home directory:

```
CFLAGS +=        -std=c++11 -fopenmp -Wall -O2 -march=native
CXXFLAGS +=      -std=c++11 -fopenmp -Wall -O2 -march=native
...
# Interpreting the POUMM parameters. Parametrizations.
From a modeling perspective, the POUMM can be regarded as a combination of two of its widely used nested models:

* The non-mixed phylogenetic Ornstein-Uhlenbeck (POU) model, (also referred to as the 'Hansen model' \cite{Butler:2004ce}), which has been used to infer local phenotypic optima in different phylogenetic clades or across discrete categories of phylogeneticall related species \cite{Butler:2004ce}, and to prove the presence of stabilizing selection towards a single stationary peak (SSP) \cite{Harmon:2010gh}.
* The phylogenetic mixed model (PMM) \cite{Lynch:1991db,Housworth:2004gj}, which has been used to measure phylogenetic heritability \cite{Hodcroft:2014es};

Thus, the POUMM combines the applications of the above two models. Besides inferring its parameters, the POUMM has several useful properties helping the interpretation of the data and allowing to make predictions about the future trait evolution of the considered population. The properties which we consider represent bijective functions of some of the POUMM parameters. Thus, it is possible to reparametrize the POUMM, so that the model inference is done directly on properties of interest, e.g. the phylogenetic heritability. This is particularly useful for Bayesian inference, since for Bayesian inference priors should be specified for the properties of interest rather than the default POUMM parameters. We call a *parametrization* any numerical bijective function mapping its argument into the default POUMM parameter-space ($<g_M,\alpha,\theta,\sigma,\sigma_e>$). 
...
\begin{align}
<g_M, \alpha,\theta,\sigma_z^2,\sigma_e> & \to<g_M, \alpha,\theta,\sigma=\sqrt{2\alpha(\sigma_{z}^2-\sigma_e^2)},\sigma_e>
\end{align}
...
\venelin{A likelihood ratio test between the ML POUMM and PMM fits can be used to test if the inferred parameter $\alpha$ is significantly above 0. As pointed out in the previous paragraph, a significantly positive $\alpha$ does not necessarily imply stabilizing selection towards $\theta$. Further, it is important to note that the value of $\alpha$ can only be interpreted with respect to the time scale of the phylogeny. It can be more intuitive to consider the phylogenetic half-life, $t_{1/2}=\frac{\ln(2)}{\alpha}$, which equals the time it takes for a species entering a new niche to evolve halfway toward its new expected optimum} \cite{Hansen:1997ek}.

## Phylogenetic heritability
The term *phylogenetic heritability*, introduced with the phylogenetic mixed model (PMM) \cite{Housworth:2004gj}, measures the proportion of phenotypic variance in a population attributable to heritable factors, such as genes, as opposed to non-heritable factors, such as environment and measurement error. Although this concept has been applied mostly in the context of the original PMM, i.e. under the assumption of Brownian motion, the same concept applies to any evolutionary model allowing for the estimation of measurement error (ME) \cite{Hansen:2012et}. The *phylogenetic heritability* is defined as the expected proportion of phenotypic variance attributable to $g$ at the tips of the tree, $\sigma^2(g)/\left[\sigma^2(g)+\sigma_e^2\right]$ \cite{Housworth:2004gj}. This definition is a phylogenetic variant of the definition of broad-sense heritability, $H^2$, from quantitative genetics \cite{Lynch:1998vx}. However, in the case of a trait evolving along a phylogeny, the expected genotypic variance, $\sigma^2(g)$, and, therefore, the phylogenetic heritability, are functions of time. Depending on the applicaiton, the following three types of phylogenetic heritability might all be of interest:

* Expectation at the mean height ($\bar{h}$): $H_{\bar{h}}^2:=\left[\sigma^2\, \frac{\left(1-e^{-2\alpha \bar{h}}\right)}{2\alpha}\right]/\left[\sigma^2\, \frac{\left(1-e^{-2\alpha \bar{h}}\right)}{2\alpha}+\sigma_e^2\right]$. \venelin{This definition gives rize to three parametrizations where $H_{\bar{h}}^2$ is a free model parameter, while one of the parameters $\alpha$, $\sigma$ or $\sigma_e$ is a function of $H_{\bar{h}}^2$ and the two other parameters. These parametrizations can be expressed as: 
\begin{align}
<g_M, \alpha, \theta, H_{\bar{h}}^2, \sigma_e> & \to<g_M, \alpha, \theta, \sigma=\sqrt{\frac{2\alpha H_{\bar{h}}^2 \sigma_e^2}{(1-e^{-2\alpha t})(1-H_{\bar{h}}^2)}}, \sigma_e>\label{eq:sigmaH2alphasigmae} \\
<g_M, H_{\bar{h}}^2, \theta, \sigma, \sigma_e> & \to<g_M, \alpha=\frac{y+W(y e^{-y})}{2\bar{h}}, \theta, \sigma, \sigma_e>\label{eq:alphaH2sigmasigmae} \\
<g_M, \alpha, \theta, \sigma, H_{\bar{h}}^2> & \to<g_M, \alpha, \theta, \sigma, \sigma_e=\sqrt{\frac{\sigma^2(1-e^{-2\alpha \bar{h}})}{2 \alpha}(\frac{1}{H_{\bar{h}}^2} - 1)}>,\label{eq:sigmaeH2alphasigma}
\end{align}
where $y=\frac{\bar{h}\sigma^2}{\sigma_e^2 H_{\bar{h}}^2} - \frac{\bar{h}\sigma^2}{\sigma_e^2}$ and $W$ is the Lambert-W function.}
* Expectation at equilibrium of the OU-process ($t\to\infty$): $H_{\infty}^2:=\lim_{\bar{h}\to\infty}H_{\bar{h}}^2=\frac{\sigma^2}{\sigma^2+2\alpha\sigma_e^2}$. \venelin{By taking the limit $\bar{h}\to\infty$ in equations \ref{eq:sigmaH2alphasigmae}, \ref{eq:alphaH2sigmasigmae} and \ref{eq:sigmaeH2alphasigma}, we obtain the corresponding parametrizations:
\begin{align}
<g_M, \alpha, \theta, H_{\infty}^2, \sigma_e> & \to<g_M, \alpha, \theta, \sigma=\sqrt{\frac{2\alpha H_{\infty}^2 \sigma_e^2}{1-H_{\infty}^2}}, \sigma_e>\label{eq:sigmaH2alphasigmaetInf} \\
<g_M, H_{\infty}^2, \theta, \sigma, \sigma_e> & \to<g_M, \alpha=\frac{\sigma^2 (1-H_{\infty}^2)}{2H_{\infty}^2\sigma_e^2}, \theta, \sigma, \sigma_e>\label{eq:alphaH2sigmasigmaetInf} \\
<g_M, \alpha, \theta, \sigma, H_{\infty}^2> & \to<g_M, \alpha, \theta, \sigma, \sigma_e=\sqrt{\frac{\sigma^2}{2 \alpha}(\frac{1}{H_{\infty}^2} - 1)}>,\label{eq:sigmaeH2alphasigmatInf}
\end{align}}
* Empirical (time-independent) version of the heritability based on the sample phenotypic variance $s^2(\textbf{z})$ : $H_e^2:=1-\sigma_e^2/s^2(\textbf{z})$. This definition is useful when the tree is non-ultrametric but there is sufficient evidence that the empirical distribution of the trait is stationary along the tree. \venelin{In this case, $s^2(\textbf{z})$ coincides with the sum of the OU variance at equilibrium and $\sigma_e^2$. The corresponding parametrization is:
\begin{align}
<g_M, \alpha, \theta, \sigma, H_{e}^2> & \to<g_M, \alpha, \theta, \sigma, \sigma_e=\sqrt{s^2(\textbf{z})(1-H_e^2)}>
\label{eq:sigmaeH2e}
\end{align}}

To learn about different parametrizations supported by the POUMM package, read the documentation page `?POUMM::specifyPOUMM`. 
