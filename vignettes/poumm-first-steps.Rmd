...
install.packages('POUMM_1.1.0.tgz', type='binary', repos=NULL)
...
```{r}
spec <- POUMM::specifyPOUMM_ATH2tMeanSe(
  z[1:N], tree,                       
  parLower = c(alpha=0, theta=0, H2tMean=0, sigmae=0),
  parUpper = c(alpha=100, theta=8, H2tMean=1, sigmae=10),
  parallelMCMC = TRUE, nSamplesMCMC=4e5)

cluster <- parallel::makeCluster(parallel::detectCores(logical = FALSE))
doParallel::registerDoParallel(cluster)

fitPOUMM3 <- POUMM::POUMM(z[1:N], tree, spec = spec)
